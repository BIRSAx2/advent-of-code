...
_AdventOCaml_ is an OCaml starter project for Advent of Code.
...
_AdventOCaml_ needs a session token from `adventofcode.com` in order to download
...
Each problem needs to conform to the `Problem.T` [interface](https://github.com/Sixstring982/AdventOCaml/blob/main/lib/problem.ml), which provides basic
...
_AdventOCaml_ will automatically register `.ml` or `.re` files in `lib/problems` which start
...
For example, if we'd like to start working on day 1 of year 2023, _AdventOCaml_
...
AdventOCaml uses [Tanenbaum](https://github.com/Sixstring982/tanenbaum) as base, with upgrades to leverage OCaml 5.2.1